...
stralloc(const char *string)
{
	...
	if (copy == NULL)
		return NULL;
	strcpy_s(copy, sizeof(string), string);
	return copy;
}
...
createMenu6(void)
{
	...
	for (i = 0; i < MODES; i++) {
		sprintf_s(str, "%srequest %s", (modes[i] ? "+ " : "   "), modeNames[i]);
...
#define REPORTSTART(text)                          \
	...
	strcpy_s(textPtr[0], sizeof(text), text);                  \
        textCount = 1;
...
makeWindow(int index)
{
	...
	case 3:
		...
		sprintf_s(str, "window %d (RGB)", index);
		glutSetWindowTitle(str);
		sprintf_s(str, "icon %d", index);
	...
	case 7:              /* color index window  */
		...
		sprintf_s(str, "window %d (color index)", index);
		glutSetWindowTitle(str);
		sprintf_s(str, "icon %d", index);
		glutSetIconTitle(str);
		glutSetMenu(menu1);
...
updateText(void)
{
	...
	if (textPtr[0] == NULL) {
		for (i = 0; i < 20; i++) {
			...
			strcpy_s(textPtr[i], sizeof("no current text"), "no current text");
		}
		textPtr[20] = NULL;
	}
